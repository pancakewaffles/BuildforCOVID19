...
def search_gify():
    """
    Randomly searches for either gifs or quotes.
    """
    p = random.random()
    ...
    if p < 0.5:
        ...
        """Get quotes"""
        no_of_quotes = 1
        result = quotes(user_input)
        print(result)
        for i in range(no_of_quotes):
            response_string +=str(result[i]) + " said the Dalai Lama."
...
"""Functions to help scrap brainyquotes"""
def quotes(user_input):
    quotes_list = []
    url = "https://www.brainyquote.com/search_results?q="+user_input+".html"
